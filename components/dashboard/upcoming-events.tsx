...
export async function UpcomingEvents() {
  ...
  const isGraduationDay =
    new Date().toDateString() === new Date('2025-07-04').toDateString();
        ...
        {eventsData.map(event => {
          ...
          return (
            <div key={event.title} className='flex items-center space-x-4'>
              <div
                className={`flex h-9 w-9 items-center justify-center rounded ${event.iconColor}`}
              >
        ...
        {isBlackOwlsStudent && isGraduationDay && (
          <div className='mt-4 p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded border border-purple-200'>
            ...
            <p className='text-xs text-purple-600 mt-1'>
              ...
              {user?.role === 'STUDENT' ? 'your bootcamp' : 'the program'}
            </p>
          </div>
        )}
      </CardContent>

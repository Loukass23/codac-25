  ...
  const statsData: StatCard[] = [
    {
      ...
      change: userStats?.coursesInProgress
        ? `${userStats.coursesInProgress} in progress`
        : 'Start learning',
      icon: BookOpen,
    },
    {
      ...
      change: userStats?.totalEnrollments
        ? `${Math.round((userStats.coursesCompleted / userStats.totalEnrollments) * 100) || 0}% completion rate`
        : 'No completions yet',
      icon: BookCheck,
    },
    {
      title: 'Study Time',
      value: userStats?.monthlyStudyTime
        ? `${userStats.monthlyStudyTime}h`
        : '0h',
      change: 'This month',
      icon: Clock,
    },

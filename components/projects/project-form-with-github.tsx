...
import { CalendarIcon, FileText, Github, PlusIcon, X } from 'lucide-react';
...
interface ProjectFormWithGitHubProps {
...
export function ProjectFormWithGitHub({
  ...
  const [importedData, setImportedData] =
    useState<Partial<CreateProjectData> | null>(null);
  ...
  const handleGitHubImport = (data: Partial<CreateProjectData>) => {
    ...
    toast.success(
      `Repository data imported successfully! ${hasReadme ? 'README has been converted to project summary.' : ''} You can now edit the details before saving.`
    );
  };
  ...
  const addTechnology = () => {
    if (
      newTech.trim() &&
      !form.getValues('techStack').includes(newTech.trim())
    ) {
        ...
        <Card>
          ...
          <CardContent>
            <Tabs
              value={creationMode}
              onValueChange={value =>
                setCreationMode(value as ProjectCreationMode)
              }
            >
                ...
                {importedData && (
                  <div className='mb-4 p-3 bg-green-50 border border-green-200 rounded-md'>
                    <p className='text-sm text-green-800'>
                      ✅ Data imported from GitHub repository. You can now edit
                      the details below.
                    </p>
                  </div>
                )}
      ...
      {(creationMode === 'manual' || projectId) && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Write a detailed summary using our rich text editor. This is where
              you can tell the full story of your project with formatting,
              images, and more.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {projectId ? (
              ...
              <div className='text-center py-8 text-muted-foreground'>
                <p>
                  Save your project first to enable the rich text summary editor
                </p>
              </div>
            )}
          </CardContent>

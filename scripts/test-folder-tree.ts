...
import { getLMSFolderTreeWithDocuments } from '../data/documents/get-folders';

async function testFolderTree() {
    try {
        console.log('Testing LMS folder tree...');

        const result = await getLMSFolderTreeWithDocuments('test-user-id');

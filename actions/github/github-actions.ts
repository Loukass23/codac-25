 ...
 */
export async function fetchGitHubRepositories(): Promise<ServerActionResult<GitHubRepository[]>> {
    try {
    ...
    try {
        ...
        const project = await prisma.projectShowcase.create({
            data: {
                ...
                description: additionalData?.description || repository.description || `A project from ${repository.name}`,
                summary: Prisma.JsonNull, // Can be populated later with rich content
                shortDesc: additionalData?.shortDesc || repository.description || repository.name,
                ...
                status: (additionalData?.status as any) || 'COMPLETED',
                startDate: additionalData?.startDate ? new Date(additionalData.startDate) : new Date(repository.created_at),
                endDate: additionalData?.endDate ? new Date(additionalData.endDate) : new Date(repository.updated_at),
        ...
        logger.error('Error creating project from GitHub repository', error instanceof Error ? error : new Error(String(error)))
 ...
 */
export async function checkGitHubConnection(): Promise<ServerActionResult<boolean>> {
    try {
        const user = await getCurrentUser()
        if (!user) {
            return {
        ...
        logger.error('Error checking GitHub connection', error instanceof Error ? error : new Error(String(error)))
        return {
            success: false,
...
export async function getRepositoryDetails(
    ...
    try {
        ...
        logger.error('Error fetching repository details', error instanceof Error ? error : new Error(String(error)))
        return {
            success: false,
 ...
 */
export async function getGitHubUserInfo(): Promise<ServerActionResult<any>> {
    try {
        const user = await getCurrentUser()
        if (!user) {
            return {
        ...
        logger.error('Error fetching GitHub user info', error instanceof Error ? error : new Error(String(error)))
        return {
            success: false,
            error: 'Failed to fetch GitHub user information'
        }
